...
class PropertyInstruction extends instruction_1.Instruction {
    ...
    getProperties() {
        if (this.properties === undefined) {
            ...
            if (args.length === 0) {
                ...
                this.properties = [new property_1.Property(this.document, this.escapeChar, args[0], new argument_1.Argument(text, text, vscode_languageserver_types_1.Range.create(args[1].getRange().start, args[args.length - 1].getRange().end)))];
            }
            else {
                this.properties = [];
    ...
    getArguments() {
            ...
            switch (char) {
                            ...
                            whitespaceCheck: for (let j = i + 2; j < content.length; j++) {
                                switch (content.charAt(j)) {
                                    ...
                                    default:
                                        ...
                                        args.push(new argument_1.Argument(content.substring(argStart, i), content.substring(argStart, i), vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + i + 2))));
                                        argStart = j;
                                        break whitespaceCheck;
                                }
                            }
                ...
                case '"':
                    ...
                    for (let j = i + 1; j < content.length; j++) {
                        switch (content.charAt(j)) {
                            case char:
                                args.push(new argument_1.Argument(content.substring(argStart, j + 1), content.substring(argStart, j + 1), vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + j + 1))));
                                i = j;
                                argStart = -1;
                                continue argumentLoop;
                            case this.escapeChar:
                ...
                case '\t':
                    if (escaped) {
                        ...
                        args.push(new argument_1.Argument(content.substring(argStart, i), content.substring(argStart, i), vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + i))));
                        argStart = -1;
                    }
                    break;
                case '\r':
                ...
                case '#':
                    if (escaped) {
                        ...
                        args.push(new argument_1.Argument(value, value, vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + mark))));
                        argStart = -1;
                        break argumentLoop;
                    }
                    else if (argStart === -1) {
                ...
                default:
                    if (spaced) {
                        if (argStart !== -1) {
                            args.push(new argument_1.Argument(content.substring(argStart, mark), content.substring(argStart, mark), vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + mark))));
                            argStart = -1;
                        }
                        spaced = false;
                    }
        ...
        if (argStart !== -1 && argStart !== content.length) {
            ...
            args.push(new argument_1.Argument(value, value, vscode_languageserver_types_1.Range.create(this.document.positionAt(instructionNameEndOffset + start + argStart), this.document.positionAt(instructionNameEndOffset + start + end))));
        }
        return args;
    }
}
