...
alias gcl="git checkout -- ." # clear unstaged changes (will not delete new files though)
alias gop="git open"
alias gundo="git reset HEAD^" # undo last commit
alias gbunny="git commit -m '(\ /)
(O.o)
...
alias 2js="pbpaste | json-to-js | cop"
alias to64="pbpaste | encode64 | tee >(cop)"
...
alias slackdev="export SLACK_DEVELOPER_MENU=true && open -a /Applications/Slack.app"
...
dstop() { docker stop $1 && docker rm $1; }
dsize() { echo $(expr $(docker image inspect $1 --format='{{.Size}}') / 1000000)MB ;}

party() {
  opt=false
  while getopts ":yn" option; do
    case $option in
      y) POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(zsh_party user dir vcs)
         opt=true
        ;;
      n) POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(user dir vcs)
         opt=true
        ;;
      ?);;
    esac
